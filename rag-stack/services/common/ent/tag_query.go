...
type TagQuery struct {
	...
	withEmbeddingCoverages *TagEmbeddingCoverageQuery
...
// QueryEmbeddingCoverages chains the current query on the "embedding_coverages" edge.
func (_q *TagQuery) QueryEmbeddingCoverages() *TagEmbeddingCoverageQuery {
	query := (&TagEmbeddingCoverageClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withEmbeddingCoverages: _q.withEmbeddingCoverages.Clone(),
...
// WithEmbeddingCoverages tells the query-builder to eager-load the nodes that are connected to
// the "embedding_coverages" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TagQuery) WithEmbeddingCoverages(opts ...func(*TagEmbeddingCoverageQuery)) *TagQuery {
	query := (&TagEmbeddingCoverageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withEmbeddingCoverages = query
	return _q
}
...
func (_q *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := _q.withEmbeddingCoverages; query != nil {
		if err := _q.loadEmbeddingCoverages(ctx, query, nodes,
			func(n *Tag) { n.Edges.EmbeddingCoverages = []*TagEmbeddingCoverage{} },
			func(n *Tag, e *TagEmbeddingCoverage) {
				n.Edges.EmbeddingCoverages = append(n.Edges.EmbeddingCoverages, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *TagQuery) loadEmbeddingCoverages(ctx context.Context, query *TagEmbeddingCoverageQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *TagEmbeddingCoverage)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tagembeddingcoverage.FieldTagID)
	}
	query.Where(predicate.TagEmbeddingCoverage(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tag.EmbeddingCoveragesColumn), fks...))
	}))
