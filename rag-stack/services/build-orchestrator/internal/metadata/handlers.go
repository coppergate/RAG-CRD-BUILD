...
func (s *Service) handleVersion(w http.ResponseWriter, r *http.Request) {
	...
	if serviceName == "" {
		s.sendJSONError(w, "Service name required", http.StatusBadRequest)
		return
	}
	...
	case http.MethodGet:
		...
		if v == nil {
			s.sendJSONError(w, "Not found", http.StatusNotFound)
			return
		}
		w.Header().Set("Content-Type", "application/json")
...
func (s *Service) handleAcquireLock(w http.ResponseWriter, r *http.Request) {
	...
	if success {
		...
		json.NewEncoder(w).Encode(map[string]string{"status": "acquired"})
	} else {
		w.Header().Set("Content-Type", "application/json")
...
func (s *Service) sendJSONError(w http.ResponseWriter, message string, code int) {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(code)
