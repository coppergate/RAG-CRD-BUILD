...
# setup-all-no-gpu.sh - Orchestrate the RAG stack deployment (no GPU/Ollama)
# To be executed on host: hierophant
#
# Derived from setup-all.sh. Omissions vs setup-all.sh:
#   - Step 1.5: Ollama deployment skipped (no inference nodes present)
#
# All other services (llm-gateway, rag-worker, embed-gateway, rag-ingestion, etc.)
# are deployed as normal. They will be in a degraded/pending state until inference
# nodes are added and Ollama is deployed separately.
...
# Create the llms-ollama namespace so RBAC can be applied even though Ollama
# itself is not deployed in this no-gpu install.
...
# Step 1.5 (Ollama deployment) is intentionally omitted:
# Inference nodes are not present in this no-gpu install.
# Deploy Ollama manually once inference nodes are added.
...
if [[ "$PULSAR_READY" != "true" ]]; then
    ...
    echo "  1. Run the full setup:  ./setup-complete-no-gpu.sh"
...
# Ollama is not deployed in no-GPU installs; disable its readiness check
# so the pod reaches Ready state and the HPA can collect CPU metrics.
$KUBECTL set env deployment/rag-ingestion-service -n $NAMESPACE OLLAMA_REQUIRED=false
...
echo "--- All Components Deployed (No GPU) ---"
echo "NOTE: Ollama is not deployed. Services requiring inference (llm-gateway,"
echo "      embed-gateway, rag-worker, rag-ingestion) will be degraded until"
echo "      inference nodes are added and Ollama is deployed."
